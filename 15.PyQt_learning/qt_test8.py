...
# @Author  :  wanghuodong  
# @note    : 布局管理（布局管理器的布局管理类非常灵活，实用。它是将组件定位在窗口上的首选方式。QHBoxLayout和QVBoxLayout是两个基础布局管理类）

import sys
# QtWidgets包含创造经典桌面风格的用户界面提供了一套UI元素的类。
from PyQt5.QtWidgets import QApplication, QWidget, QToolTip, QPushButton, QMessageBox,QDesktopWidget,QMainWindow
from PyQt5.QtWidgets import QAction, qApp,  QTextEdit,QHBoxLayout, QVBoxLayout
...
class Example(QWidget):
    ...
    def initUI(self):
        ...
        # 创建一个水平箱布局
        hbox = QHBoxLayout()
        # 水平方向，增加一个拉伸因子，这样会在两个按钮的前面增加一个可伸缩空间，将按钮推到最右边
        hbox.addStretch(1)
        hbox.addWidget(btn)
        ...
        # 创建一个垂直箱布局
        vbox = QVBoxLayout()
        # 在垂直箱布局中增加一个拉伸因子，将把其他组件推到底部
        vbox.addStretch(1)
        # 将水平布局放置在垂直布局中，从而实现 水平布局中的组件 一直在右下角的位置
        vbox.addLayout(hbox)
